...
public class BoardController extends CommonWebUtils{
	...
	private void exceptionMsg(Exception e, ModelAndView mav, HttpServletRequest request) {
		...
		switch(e.getMessage()){
		case "E01" :
			redirectView(mav, "로그인이 필요한 서비스입니다.||로그인 페이지로 이동 하시겠습니까?", getReferer(request),"/portal/login/login.do?redirectUrl="+request.getRequestURL());			
			break;
		case "E02" :
	 ...
	 * @throws Exception
	 */	
	@RequestMapping(value="/portal/boardListJson",method=RequestMethod.POST)
