var main = {
	...
	noticeList : function(){
		...
		ajaxCall(params,'/portal/boardListJson.do');
			...
			if(result.RESULT_CODE == 'SUCCESS'){
				...
				$(result.result).each(function(i,data){
					if(i == 0){
						...
						html[++h] = '	<span><img src="/assets/portal/images/main/maIn_newsletter.png" alt="" width="74" height="96"></span>';
						html[++h] = '	<dl>';
						html[++h] = '	<dt><a href="/portal/notify/newsletter/boardView.do?board_key='+data.board_key+'">'+data.title+'</a></dt>';
				...
				$(result.result).each(function(i,data){
					if(i > 0){
						html[++h] = '<li><span class="subject"><a href="/portal/notify/newsletter/boardView.do?board_key='+data.board_key+'">'+data.title+'</a></span><span class="date">'+data.displayRegDate+'</span></li>';
					}									
				});
				html[++h] = '</ul>';
				// 뉴스레터목록
